...
/**
 * bp_like_ult_activity_filter()
 *
 * Adds activity stream filter options for 'Update Likes' and 'Blog Post Likes'.
 *
 */
function bp_like_ult_activity_filter() {
    echo '<option value="activity_liked">' . bp_like_ult_get_text('update_likes') . '</option>';

    if ( bp_like_ult_get_settings( 'enable_blog_post_support' ) == 1 ) {
      echo '<option value="blogpost_liked">' . bp_like_ult_get_text('show_blogpost_likes') . '</option>';
    }
}

add_action( 'bp_activity_filter_options' , 'bp_like_ult_activity_filter' );
add_action( 'bp_member_activity_filter_options' , 'bp_like_ult_activity_filter' );
add_action( 'bp_group_activity_filter_options' , 'bp_like_ult_activity_filter' );

/**
 * bp_like_ult_post_to_stream()
 *
 * Posts to stream, depending on settings
 *
 */
function bp_like_ult_post_to_stream( $item_id , $user_id, $group_id ) {
    ...
    if ( bp_like_ult_get_settings( 'post_to_activity_stream' ) == 1 ) {
        ...
        if ( $user_id == $author_id ) {
            ...
            $action = bp_like_ult_get_text( 'record_activity_likes_own' );

        } elseif ( $user_id == 0 ) {
            $action = bp_like_ult_get_text( 'record_activity_likes_an' );
        } else {
            $action = bp_like_ult_get_text( 'record_activity_likes_users' );
        }
