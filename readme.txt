...
3. Search for `BuddyPress Like Ultimate` and find this plugin in the results and press 'Install'
4. Press 'Activate Plugin'

= Manual Installation =

1. Upload `buddypress-like-ult` to the `/wp-content/plugins/` directory
...
- removed inline comment at top of bplike_ult.js
...
* Added function: bplikes_ult_view_who_likes().
* Added hook: bplikes_ult_view_who_likes, will be adding more.
* Added function bp_like_ult_get_num_likes() to return number of likes of an item.
...
* Function added: bp_like_ult_post_to_stream() with some code clean up inside
* Tidied up bp_like_ult_list_scripts() function, properly registering and enqueuing the scripts.
...
* Removed bp_like_ult_insert_head() as it's not needed anymore thanks to BuddyPress' theme compatibility.
* Added bp_like_ult_users_who_like() function to output the users who liked an item.
...
* Bug fixed: Could not save 'Likers Visibility' options from the BuddyPress Like Ultimate settings screen.
...
= 0.1.5 =
Many fixes, but still only suitable for testing. Settings moved to Setting->BuddyPress Like Ultimate. Can now like comments properly and remove the favourite button.
